...
bounded read/connect timeout PR (#41): the retry is only defensible because
each account read now bounds NO-PROGRESS stalls, so the failure mode that cost
the cycle costs seconds instead of ~82s (see EVIDENCE latency budget — a
nominal budget for that stall mode, NOT a proven ceiling). The two land together; the
...
Nominal preflight budget for the no-progress stall mode (DERIVED, NOT a
guarantee — see the honesty note below). The max-time failing attempt
...
**≈ 94s nominal ≪ the ~12-min (720s) intraday cadence.**

**What this figure is NOT.** It is not a guaranteed end-to-end ceiling. Requests'
`timeout` is not a wall-clock limit on a request; it fires only when NO bytes
arrive on the socket for the interval, so a peer that keeps trickling bytes can
outlast it without bound
`[VERIFIED — measured 30.1s of wall clock under `timeout=(5,10)` against a local
server sending 1 byte every 2s, 3× the read timeout and still climbing when the
probe was cut off; https://requests.readthedocs.io/en/latest/user/quickstart/#timeouts]`.
So 94s is the budget for the stall mode this PR exists to fix — a socket
delivering nothing, which is what the 07:01 incident was — and a trickling peer
is a DIFFERENT, still-unbounded mode that neither PR closes.
...
therefore unsafe; #41's `(5s, 10s)` timeout is what makes the no-progress stall
cost seconds instead of ~82s, bringing that mode's budget to a nominal ~94s.
It does not make the cycle provably bounded — see the honesty note above. Do not raise
...
| per-read cap on a NO-PROGRESS stall | `(connect 5s + read 10s) = 15s` | [VERIFIED — paired #41 timeout `(5.0, 10.0)`; inactivity timer, not total time] |
| nominal budget, no-progress stall mode | `3×2×15 + 2×2 = 94s` ≪ 720s cadence | [DERIVED — from the two rows above; NOT a wall-clock guarantee] |
| trickling peer outlasts the read timeout | 30.1s wall clock under `timeout=(5,10)`, still climbing | [VERIFIED — local 1-byte-per-2s server probe] |
...
scope:         "this is the P-BROKER-CONNECT preflight check (prod), vs existing best = today's single-try, no-retry gate that HARD-aborts the cycle on one transient blip; the bounded retry (3 attempts, 2 reads/attempt, ~94s nominal budget for the no-progress stall mode with #41's (5s,10s) inactivity timeout, not a proven wall-clock ceiling) recovers from transient blips while preserving the identical fail-closed HARD-False on a genuine outage"
