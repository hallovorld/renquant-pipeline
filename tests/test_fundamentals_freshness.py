...
Covers the 2026-06-29 fix:
  * Fix 1 — freshness measured against the quarterly FILING calendar, not a
    fixed day-count from the period date. Q1 (Mar 31) in late June is the
    latest-expected-filed quarter → PASS; data stuck multiple quarters behind
    the calendar → FAIL; the quarter-boundary case.
  * Fix 2 — a sell-only run is NOT aborted by this buy-only gate (downgrades to
    a logged soft pass), while a full/buy run still hard-fails and blocks buys.
    Safety-invariant gates are unaffected (covered elsewhere).
"""
...
# ── Fix 1: latest-expected-filed-quarter calendar ──────────────────────────
...
# ── Fix 1: classify_freshness (pure decision) ──────────────────────────────
def test_classify_pass_at_latest_expected_quarter():
    ok, msg, details = classify_freshness(date(2026, 3, 31), date(2026, 6, 29), _LAG)
    assert ok is True
    assert details["quarters_behind"] == 0
    assert "fresh" in msg


def test_classify_fail_one_quarter_behind_blocks_buys():
    # Mid-August, panel stuck at Q1 while Q2 is expected → FAIL, blocks buys.
    ok, msg, details = classify_freshness(date(2026, 3, 31), date(2026, 8, 16), _LAG)
    assert ok is False
    assert details["quarters_behind"] == 1
    assert "blocking new buys" in msg


def test_classify_fail_broken_refresh_multiple_quarters_behind():
    ok, msg, details = classify_freshness(date(2025, 9, 30), date(2026, 6, 29), _LAG)
    assert ok is False
    assert details["quarters_behind"] == 2
    assert "broken refresh" in msg


def test_classify_boundary_mid_quarter_gap_is_fresh_not_stale():
    # THE bug being fixed: in the mid-quarter filing gap the latest available
    # quarter IS the expected one, even though calendar-day age >> 45.
    # Late June, panel at Q1 (Mar 31) → 90 calendar days old but FRESH.
    today = date(2026, 6, 29)
    age_days = (today - date(2026, 3, 31)).days
    assert age_days > 45  # the old fixed-45d rule would have hard-failed here
    ok, _, _ = classify_freshness(date(2026, 3, 31), today, _LAG)
    assert ok is True


def test_classify_none_panel_is_skip():
    ok, msg, _ = classify_freshness(None, date(2026, 6, 29), _LAG)
    assert ok is True
...
# ── Fix 2: sell-only must NOT abort on this buy-only gate ───────────────────
def _patch_panel(monkeypatch, tmp_path, last_period, today):
    """Point the task at a tmp parquet with the given latest period + freeze today."""
...
def test_task_stale_full_run_hard_fails_blocks_buys(monkeypatch, tmp_path):
    # Mid-August, panel stuck at Q1 → genuinely one quarter behind.
...
def test_task_stale_sell_only_run_does_not_abort(monkeypatch, tmp_path):
    # Same genuinely-stale panel, but sell-only → soft pass (run proceeds).
    _patch_panel(monkeypatch, tmp_path, "2026-03-31", date(2026, 8, 16))
...
def test_task_fresh_at_latest_quarter_passes_in_full_run(monkeypatch, tmp_path):
    # The live 2026-06-29 case: panel at Q1, today late June → fresh → PASS.
